...
class FederatedNoOutputHandlerDiffblueTest {
   ...
   * </ul>
   * <p>
   * Method under test: {@link FederatedNoOutputHandler#doOperation(Operation, Context, Store)}
   */
  @Test
  ...
  void testDoOperation_givenExecute_thenReturnNull() throws OperationException {
    ...
    ExportToOtherAuthorisedGraph exportToOtherAuthorisedGraph = mock(ExportToOtherAuthorisedGraph.class);
    ...
    // Act
    Void actualDoOperationResult = federatedNoOutputHandler.doOperation(exportToOtherAuthorisedGraph, context, store);

    // Assert
    verify(exportToOtherAuthorisedGraph).getOption(eq("gaffer.federatedstore.operation.graphIds"));
